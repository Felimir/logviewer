...
class LogEntry:
    ...
    def render_plain_text(self):
        ...
        out = f"Ticket creado el {thread_create_time}\n"

        if self.creator == self.recipient:
            out += f"[R] {self.creator} "
            out += f"({self.creator.id}) creó un ticket. \n"
        else:
            out += f"[M] {self.creator} "
            out += f"creó un ticket con [R] "
        ...
        if not self.open:
            ...
            out += f"Ticket cerrado el {closed_time} \n"
